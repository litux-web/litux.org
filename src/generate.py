...
def process_file_if_changed(
  data_root, data_folder, data_file, html_folder, html_file
):
  global CMDLINE_ARGS
  if not CMDLINE_ARGS.force:
    data_path = os.path.join(data_root, data_folder, data_file)
    html_path = os.path.join(HTML_FOLDER, html_folder, html_file)
...
def process_local_files(html):
  global CACHE_MTIME
  while True:
    matches = re.match(
      r'(.*)(<link[^>]+href="/)([^"\?]+)("[^>]+>)',
      html, flags=re.S | re.DOTALL)
    if not matches:
      break
    mtime = CACHE_MTIME.get(matches[3], None)
    if not mtime:
      mtime = int(os.stat(os.path.join(HTML_FOLDER, matches[3])).st_mtime)
      CACHE_MTIME[matches[3]] = mtime
      print("matches", matches[3], mtime)
    html = re.sub(
      r'(.*)(<link[^>]+href="/)([^"\?]+)("[^>]+>)',
      r'\1\2\3?s='+str(mtime)+r'\4',
      html, flags=re.S | re.DOTALL)
  return html
...
def process_file(data_root, data_folder, data_file, html_folder, html_file):
  data_path = os.path.join(data_root, data_folder, data_file)
  html_path = os.path.join(HTML_FOLDER, html_folder, html_file)
  ...
  body = preparse(body)
  # if data_file == "test.md":
  #   with open("foo1", "w") as h: h.write(body)
  ...
  # if data_file == "test.md":
  #   with open("foo2", "w") as h: h.write(body)
  ...
  html_path_folder = os.path.join(HTML_FOLDER, html_folder)
...
def process_data(data_root):
  return _process_data(data_root, "", "")


def _process_data(data_root, data_folder, html_folder):
  processed = 0
